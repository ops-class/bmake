= Debian Packaging for `bmake`

This repository contains scripts for building and packaging a binary version
of `bmake` compatible with http://os161.eecs.harvard.edu/[`OS/161`] that can
be uploaded and distributed through a Launchpad PPA.
...
Repackage `bmake` only when its sources have changed.
//
...
Here is the versioning convention that we have established for `bmake`:
`20101215~xenial0`, the first package for Xenial based on `bmake` snapshot
`20101215`.
//
